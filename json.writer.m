...
:- type json_writer(Stream)
    ---> json_writer(
        json_writer_stream          :: Stream,
        json_writer_member_filter   :: json_member_filter
    ).
...
:- pred raw_put_object(json_writer(Stream)::in, json.object::in,
    State::di, State::uo) is det.


raw_put_object(Writer, MembersMap, !State) :-
    Stream = Writer ^ json_writer_stream,
    MemberFilter = Writer ^ json_writer_member_filter,
    put(Stream, '{', !State),
    get_filtered_members(MemberFilter, MembersMap, MembersList),
    raw_put_members(Writer, MembersList, !State),
    put(Stream, '}', !State).


:- pred get_filtered_members(json_member_filter::in, map(string, json.value)::in,
    list(pair(string, json.value))::out) is det.

get_filtered_members(no_member_filter, MembersMap, MembersList) :-
    MembersList = map.to_assoc_list(MembersMap).  % ← esto está bien, solo cambia el tipo arriba



get_filtered_members(member_filter(FilterPred0), MembersMap, MembersList) :-
    % Adaptamos el predicado a su forma con modo explícito
    FilterPred = (pred(K::in, V::in) is semidet :- FilterPred0(K, V)),
